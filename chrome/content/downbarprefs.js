...
function init() {
	...
	var csp = new DsbCSProvider({
		hostContainer: 'localToCloudHostContailer',
		hostSelect: 'localToCloudHost'
	});
	csp.populate();

	/* sync host list when additional hosts are enabled/disabled */
	document.getElementById('cloudAdditionalHosts').addEventListener('click', function(e) {
		csp.populate(e.target.checked);
	}, false);

	document.getElementById('localToCloudFile').addEventListener('change', function(e) {
