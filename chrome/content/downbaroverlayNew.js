...
function _dlbar_init() {
        ...
        */
        ...
        AddonManager.getAddonByID("{D4DD63FA-01E4-46a7-B6B1-EDAB7D6AD389}", function(addon) {
                ...
                var currVersion = addon.version;
                var oldVersion = 0;

                try {
                        oldVersion = _dlbar_pref.getCharPref("downbar.function.version"); // needs to be last because it's likely not there (throws error)
                } catch (e) {}
        ...
        // The default hide key is CTRL+SHIFT+z for hide,  CRTL+SHIFT+ Insert for undo clear
        ...
        var csp = new DsbCSProvider({
                hostContainer: '_dlbar_ttCloudHostContainer',
                hostSelect: '_dlbar_ttCloudHost'
        });
        csp.populate();
...
function _dlbar_hideOnBlur() {
        window.setTimeout(function() { _dlbar_blurWait(); }, 100);
}
...
//d(remaintime + "   " + (currpercent + "%") + "   " + currSize + "   " + newrate);
        ...
        if(_dlbar_speedColorsEnabled) {
                ...
                if(_dlbar_useGradients)
                        progElem.firstChild.firstChild.setAttribute("style", "background:-moz-linear-gradient(top, rgba(255, 255, 255, .8), rgba(255, 255, 255, 0.4), rgba(255, 255, 255, 0), rgba(255, 255, 255, 0)), " + newcolor + "; border-right:0px solid transparent");
...
function _dlbar_startShowFile(idtoshow, completed) {

        if(_dlbar_useAnimation && completed) {
                ...
                window.setTimeout(function(){_dlbar_finishShow(idtoshow, completed);}, 50);
        }
        else {
                _dlbar_finishShow(idtoshow, completed);
        }
}
...
function _dlbar_finishShow(idtoshow, completed) {
...
function _dlbar_setStyles() {
        ...
        if(styleDefault) {
                ...
                if(_dlbar_useGradients) {
                        dlItemTemplate.firstChild.firstChild.setAttribute("style", "background:-moz-linear-gradient(top, rgba(255, 255, 255, .8), rgba(255, 255, 255, 0.4), rgba(255, 255, 255, 0), rgba(255, 255, 255, 0)), #89AFDB;");
                        //dlItemTemplate.firstChild.firstChild.setAttribute("style", "background-image:url(chrome://downbar/skin/whiteToTransGrad.png);");
                        dlItemTemplate.setAttribute("style", "background:-moz-linear-gradient(top, rgba(255, 255, 255, .8), rgba(255, 255, 255, 0.4), rgba(255, 255, 255, 0), rgba(255, 255, 255, 0)), #89AFDB;");
...
/**/
        ...
        /* start pending cloud upload */
        var csp = _dlbar_cloudsaveQueue[localFile.path];
        if (csp) {
                csp.save([localFile.path], true);
                delete _dlbar_cloudsaveQueue[localFile.path];
        }
}
...
function _dlbar_hideRedirPopup(aEvent) {
        ...
        */
                ...
                try {
                        var matchTarget;
                        if(relTarget)
                                matchTarget = relTarget;
                        else
                                matchTarget = expOriTarget;
                        ...
                        if(matchTarget.id.substring(0,13) == "_dlbar_finTip" | matchTarget.id.substring(0,14) == "_dlbar_progTip" ||
                                /* cloud header */
                                /^_dlbar_tt/.test(matchTarget.id) ||
                                /* cloud host menu items */
                                /^menu/i.test(matchTarget.nodeName) && /^_dlbar_tt/.test(matchTarget.parentNode.id))
                                return;
                        ...
                        if(_dlbar_currTooltipAnchor.id == matchTarget.id)
                                return;
...
*/
        ...
        try {
                ...
                if (!relTarget) {
                        ...
                        /* mouse is probably on the cloud host dropdown items */
                        if (/^_dlbar_/.test(relTarget.id) || /* move from tooltip to dropdown list */
                                /* move from dropdown list to tooltip,
                                seems that moving from dropdown list to the tooltip and to the outside of the tooltip are
                                indistinguishable, for now I'll let the tooltip stay, for the user can always click
                                anywhere outside the tooltip to dismiss it */
                                /^menuitem$/i.test(relTarget.nodeName) && /^_dlbar_tt/.test(relTarget.parentNode.id))
                                return;
                }
        }
        document.getElementById("_dlbar_finTip").hidePopup();
...
function _dlbar_checkMiniMode() {
        ...
        if(_dlbar_miniMode) {
                ...
                if(currDownbar == "hbox") { // convert to miniMode
                        ...
                        if (!miniElem) {
                                // It's not anywhere - see if the add-on bar is visible and if so, put it there
                                var addonBar = document.getElementById("addon-bar");
                                if (addonBar && !addonBar.collapsed) {
                                        var addonBarCloseButton = document.getElementById("addonbar-closebutton")
                                    addonBar.insertItem("downbarMini", addonBarCloseButton.nextSibling);
                                }
                                else {
                                        // Add-on bar is collapsed or we are using ffox3.6(before addon-bar), attempt to put it on the end of the nav bar
                                        var navBar = document.getElementById("nav-bar");
                                        if (!navBar.collapsed) {
                                                navBar.insertItem("downbarMini", null, null, false);
                                        }
                                        else {
                                                // Both the addon bar and nav bar are not shown, put it on the addon bar and make it visible
                                                var addonBarCloseButton = document.getElementById("addonbar-closebutton")
                                        addonBar.insertItem("downbarMini", addonBarCloseButton.nextSibling);
                                        addonBar.collapsed = false;
                                        }
                                }
                                miniElem = document.getElementById("downbarMini");
                        }
                        else if(miniElem.parentNode.collapsed) {  // if it's on a toolbar but the toolbar is collapsed, show it
                                miniElem.parentNode.collapsed = false;
                        }
...
function _dlbar_showMiniPopup(miniElem, event) {
        ...
        if(event.button == 0) {
                document.getElementById("downbarPopup").showPopup(miniElem,  -1, -1, 'popup', 'topleft' , 'bottomleft');
        }
