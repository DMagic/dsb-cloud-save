var ModernDropbox = function(consumerKey, consumerSecret) {
	...
	var _setupAuthStorage = function() {
		...
		for (i in keys) {
			var key = keys[i];
			value = cloudSavePreference.getItem(_storagePrefix + key);
			if (value) {
				_tokens[key] = value;
			}
		}
	...
	var _clearAuthStorage = function() {
		...
		for (i in keys) {
			var key = keys[i];
			cloudSavePreference.removeItem(_storagePrefix + key);
		}
	};
	...
	var _storeAuth = function(valueMap) {
		...
		for (i in keys) {
			...
			if (valueMap[key] !== undefined) {
				...
				cloudSavePreference.setItem(_storagePrefix + key, valueMap[key]);
