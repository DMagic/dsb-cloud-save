...
ChromeExOAuth.prototype.authorize = function(callback) {
  ...
  function list(tabId, changeInfo, tab) {
    if (changeInfo.url && changeInfo.url.indexOf('chromeexoauthcallback') != -1) {
        if (isChrome) chrome.tabs.remove(tabId);
				else setTimeout(function() { tab.close() }, 100);
    ...
    var params = ChromeExOAuth.formDecode(querystring);
      ...
      if (isChrome) chrome.tabs.onUpdated.removeListener(list);
			else FxTabUpdateListener.removeListener(list);
    }
  }
  if (isChrome) {
		...
		FxTabUpdateListener.addListener(list);
...
ChromeExOAuth.prototype.clearTokens = function() {
  cloudSavePreference.removeItem(this.key_token + encodeURI(this.oauth_scope));
  cloudSavePreference.removeItem(this.key_token_secret + encodeURI(this.oauth_scope));
};
...
ChromeExOAuth.initCallbackPage = function() {
  var background_page = isChrome ? chrome.extension.getBackgroundPage() : window;
...
ChromeExOAuth.sendRequest = function(method, url, headers, body, callback) {
  ...
  xhr.upload.addEventListener('progress', function(evt){
		uploadProgress(isChrome ? 'todo_fix_this' : url, evt);
  }, false)
...
ChromeExOAuth.prototype.setToken = function(token) {
  cloudSavePreference.setItem(this.key_token + encodeURI(this.oauth_scope), token);
};
...
ChromeExOAuth.prototype.getToken = function() {
  return cloudSavePreference.getItem(this.key_token + encodeURI(this.oauth_scope));
};
...
ChromeExOAuth.prototype.setTokenSecret = function(secret) {
  cloudSavePreference.setItem(this.key_token_secret + encodeURI(this.oauth_scope), secret);
};
...
ChromeExOAuth.prototype.getTokenSecret = function() {
  return cloudSavePreference.getItem(this.key_token_secret + encodeURI(this.oauth_scope));
};
...
ChromeExOAuth.prototype.getAccessToken = function(oauth_token, oauth_verifier,
  ...
  var bg = isChrome? chrome.extension.getBackgroundPage() : window;
...
/**
 * Firefox tab event listener
 */
if (isFirefox) {
	var FxTabUpdateListener =
	{
		...
		{
			var wnd = aBrowser.contentWindow, domWnd = wnd.wrappedJSObject;
			for (var k in this.callbacks)
				this.callbacks[k](domWnd, { url: domWnd.location.href }, wnd);
		},
	...
	gBrowser.addTabsProgressListener(FxTabUpdateListener);
}